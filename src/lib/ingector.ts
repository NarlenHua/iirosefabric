...
export class Ingector {
  api!: FabricAPI;
  ingectorStyle = ingectorStyle;
  iirosesave = iirosesave;
  iiroserepair = iiroserepair;
  iiroseremovecaches = iiroseremovecaches;
  ...
  // 创建一个表格的一行
  createTabletr(name: HTMLElement, link: HTMLElement, choice: HTMLElement) {
    let tabletr = this.api.windowElementer.createItem('tr');
    let nametd = this.api.windowElementer.createItem('td', undefined, ingectorStyle.class["ingector-linkname"]);
    nametd.append(name);
    let linktd = this.api.windowElementer.createItem('td', undefined, ingectorStyle.class["ingector-link"]);
    linktd.append(link);
    let choicetd = this.api.windowElementer.createItem('td', undefined, ingectorStyle.class["ingector-choice"]);
  ...
  // 创建一个外部资源表格
  async createTable(tableType: number, writeButton: HTMLElement, readButton: HTMLElement, addButton: HTMLElement): Promise<HTMLElement> {
    ...
    let table = this.api.windowElementer.createItem('table', undefined, ingectorStyle.class["ingector-table"]);
    let titleTemp = ''
    switch (tableType) {
      case 0:
        titleTemp = '加载后注入的js';
    ...
    let tablebody = this.api.windowElementer.createItem('tbody');
    let tabletr = this.createTabletr(writeButton, readButton, addButton)
    tablebody.append(tabletr);
    table.append(tablebody);
    let addline = async (lineValue?: [string, string, number]) => {
      let name: HTMLInputElement = this.api.windowElementer.createItem('input') as HTMLInputElement;
      let link = this.api.windowElementer.createItem('input') as HTMLInputElement;
      let choice = this.api.windowElementer.createItem('button') as HTMLButtonElement;
      ...
      let newTabletr = this.createTabletr(name, link, choice);
   ...
   */
  async creatIngectorWindow() {
    // 二级菜单
    let menuItem = this.api.windowElementer.createMenuItem('设置注入的资源');
    // 工作区
    let workSpace: HTMLElement = this.api.windowElementer.createItem('div', `ingectorjsWindow-workspace`, fabricStyle.class["fabric-window-workspace"])
    let table0 = await this.createTable(
      0,
      this.api.windowElementer.createItem('button', undefined, undefined, '写入'),
      this.api.windowElementer.createItem('button', undefined, undefined, '读取'),
      this.api.windowElementer.createItem('button', undefined, undefined, '添加')
    );
    let table1 = await this.createTable(
      1,
      this.api.windowElementer.createItem('button', undefined, undefined, '写入'),
      this.api.windowElementer.createItem('button', undefined, undefined, '读取'),
      this.api.windowElementer.createItem('button', undefined, undefined, '添加')
    );
    let table2 = await this.createTable(
      2,
      this.api.windowElementer.createItem('button', undefined, undefined, '写入'),
      this.api.windowElementer.createItem('button', undefined, undefined, '读取'),
      this.api.windowElementer.createItem('button', undefined, undefined, '添加')
    );
    let disc = this.api.windowElementer.createItem('p', undefined, undefined, '使用方法,设置选项后并不会立刻保存，需点击写入才可以，所有资源下载进入才能加载。可以设置三种状态，删除将会在写入后移除，保存会在写入后保存在本地，禁用将不会启动。写入是将当前设置的写入到本地，这样设置的才起作用。');
    let disc0 = this.api.windowElementer.createItem('p', undefined, undefined, '大多数js脚本都可以使用这个，它运行在界面加载后。');
    let disc1 = this.api.windowElementer.createItem('p', undefined, undefined, '大多数css文件都可以使用这个，它的加载也在界面加载后。');
    let disc2 = this.api.windowElementer.createItem('p', undefined, undefined, '下面设置的脚本，在界面加载时会运行，它在下次启动后会写入到缓存，即使删掉fabric也依旧会运行，这里的脚本下下次启动才能运行。');
    workSpace.append(disc, disc0, table0, disc1, table1, disc2, table2);
    let fabiricMianWindow = this.api.windowElementer.createFabrcWindow('ingectorjsWindow', 300, workSpace, '设置注入的资源', 400);
    // 关闭窗口
    this.api.windowElementer.closeElement(fabiricMianWindow);
    console.log('窗口', fabiricMianWindow);
    menuItem.onclick = () => { this.api.windowElementer.turnDisplay(fabiricMianWindow); };
    this.api.iiroseElements.movePanelHolder?.appendChild(fabiricMianWindow);

    // 返回控制它的二级菜单
    return menuItem;
  }
  async initIngector(tempAPI: FabricAPI) {
    this.api = tempAPI;
  }
}