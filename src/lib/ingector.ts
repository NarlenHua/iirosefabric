...
async function runBegain() {
   ...
   * @param error Error对象
   */
  // window.addEventListener('error', function (errorTemp) {
  //   console.error('error对象', errorTemp);
  //   // @ts-ignore
  //   console.error(' errorTemp.stack', errorTemp.stack, 'errorName', errorTemp.name, errorTemp.filename, errorTemp.lineno, errorTemp.eventPhase)

  // })
  window.onerror = function (message, source, lineno, colno, error) {
    console.error(`消息"${message}"`, '错误脚本的链接', source, '错误行号', lineno, '错误列号', colno, '错误对象', `[[[${error?.toString()}]]]`);
    // TypeError: a.UnkonwMessage is not a constructor
    // at Object.decodeMessage (iiroseFabric0.0.2.user-1217307009.js:1:2085)
    // at M.socket.afterOnmessage (iiroseFabric0.0.2.user-1217307009.js:1:23398)
    // at M.socket.onmessage (iiroseFabric0.0.2.user-1217307009.js:1:23632)
    // at WebSocket._onmessage (media_play.js?_=1721572765800:4219:24)
    // at socket.reader.onload (eval at <anonymous> (messages.html:1:1), <anonymous>:1:642174)
